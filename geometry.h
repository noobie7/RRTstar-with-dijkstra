#ifndef GEOMETRY_H
#define GEOMETRY_H
// ^To make sure I don't declare any function more than once by including the
...
/*  NOTE: Most of the calculations are done using EPS as a factor of difference
    since double/long double doesn't store floating point values precisely
   (limited precision) */
...
struct Point {
  ...
  Point operator/(ftype t) const { return Point(*this) /= t; }
  ftype dot(const Point &t) const { return (x * t.x + y * t.y); }
  ftype cross(const Point &t) const { return x * t.y - y * t.x; }
  ...
  bool operator==(const Point &rhs) const {
    return fabs(x - rhs.x) < EPS &&
           fabs(y - rhs.y) < EPS; // or another approach as above
  }
};
...
ftype cross(Point a, Point b) { return (a.x * b.y - b.x * a.y); }
...
bool checkCollision(Point lineFrom, Point lineTo, Point location,
  ...
  location += Point(
      radius,
      radius); // Adjust location from top-left corner to center coordinates
  ftype ab2, acab, h2;
...
// https://stackoverflow.com/questions/11716268/point-in-polygon-algorithm this
// can be done in log(N) though
...
// helper function
int sign(const ftype x) { return x >= 0 ? x ? 1 : 0 : -1; }
...
bool check_intersection(const Point a, const Point b, const Point c,
                        const Point d) {
